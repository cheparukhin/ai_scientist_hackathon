...
Hide a known failure's label, paste it as a candidate, show 2D similarity misses the link, off-target class matching recovers the liability, and the plan reorders. **Primary leave-one-out demo: rimonabant** (CB1, z=+10.8, default rank 15 -> rank 1) — under strict LOO the CB1 signal survives because ChEMBL has many *other* CB1 antagonists resembling it, so the tool re-derives the liability from molecules that never failed. **Pergolide is best shown in normal mode, NOT demo mode**: normally it correctly headlines 5-HT2B valvulopathy (rank 11 -> 2), with clinical grounding out-prioritizing its higher raw-similarity *therapeutic* D2 target — a clean "consequence beats likelihood" story. Under strict LOO, though, pergolide's 5-HT2B signal collapses (it was carried entirely by its withdrawn ergot cousins, which LOO also hides), so demo-mode pergolide honestly reports it could not recover the mechanism — the app now says exactly this rather than mislabeling the recovered D2 hit as a win. Terfenadine/hERG is kept only as a clean validation anchor (z=+6.4), **not** the headline. Then a trust beat: paste cyclosporine -> **abstains**; paste a close known analog -> **known analog flag**.
