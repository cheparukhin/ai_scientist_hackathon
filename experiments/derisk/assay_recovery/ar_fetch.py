...
# 14-target safety panel: name -> ChEMBL target id
TARGETS = {
...
def fetch_target(tid, want=900):
    ...
    while len(mols) < want and pages < 8:
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "ar-test"})
...
with open(os.path.join(OUT, "ar_actives.json"), "w") as f:
