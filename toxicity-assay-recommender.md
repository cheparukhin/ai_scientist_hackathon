...
- **Output — a reordered in-vitro assay plan optimized for time-and-cost-to-kill:**
  1. **The headline — a re-ordered safety-assay plan:** the assay most likely to catch a real, program-ending liability moved to the front, each tagged with an **actionability label** (*no-go / counter-screen / monitor*, §4 Layer 3b) and an **evidence tier** (*mechanism-linked / model-predicted / abstain*). The value is quantified as **assays-to-culprit** — how far down the default plan the killer experiment sits vs. where we put it. E.g. *"5-HT2B counter-screen first (no-go: resembles drugs withdrawn for valvulopathy) — default panels bury it at rank 11; we move it to rank 1."*
...
Under the hood it's a **tiered portfolio** (§4): a validated off-target-mechanism engine (M1, the core) + wrapped outcome-models (M2) for the metabolism-driven liver/mito gap. It is **decision-support / experiment-prioritization**, not a binary safety classifier — more honest (proxies miss metabolism, dose, exposure) and directly aligned with the economic goal: **reach the kill decision with fewer experiments.**
...
> **Be precise about T1 vs T2 in the pitch:** the *clinical-failure grounding* (T1) is what's novel, but the *scoring backbone* is largely **T2 organ-liability labels** (DILIrank/DICTrank — marketed drugs) plus the off-target similarity engine. Don't imply the whole score comes from toxicity-causal clinical failures — T1 is the grounding badge, T2 + R4 do the statistical lifting.
...
| **WITHDRAWN 2.0** (Charité) | Withdrawn/discontinued drugs + withdrawal reasons | ~578 (v1) → ~631–635 (2.0) | Free — cheminfo.charite.de/withdrawn; NAR 2024 52:D1503 | ⚠️ **Includes *non-safety* withdrawals**. Must filter on `safety_related` + organ before calling anything T1. |
...
| **ChEMBL** | measured bioactivities incl. safety targets (hERG, 5-HT2B, BSEP…) | ~2.4M cpds | **the mechanism layer** — reference drugs' *known* off-targets; MCP available; **the R4 engine runs on this** |
...
For a candidate, rank the safety-panel assays by predicted-liability so the **program-ending assay is first**. Success = **low assays-to-culprit** (rank of the killer assay) on held-out real failures, vs. a defensible default panel order. Everything below serves that metric.

### Empirical reality check #1 — 2D structure can't link mechanistic failures (drove the design)
7 pairs of drugs **both withdrawn/failed for the same organ toxicity via the same documented mechanism**; 2D fingerprints miss all of them (ECFP4 0.05–0.19; feature/pharmacophore FCFP only 1.3–1.5× lift). The signal linking them is a **shared binding target**, invisible to 2D structure. USRCAT 3D shape was tested and is a **weak supplement** (helped 4/7, hurt 3/7 — global shape dominates over the target-relevant toxicophore). **Conclusion: the retrieval engine must be off-target / mechanism similarity (R4), not 2D structure.** (Scripts: `run_all.py`, `run_pharm.py`, `usrcat_experiment.py`.)

### Empirical reality check #2 — R4 class-aggregation recovers the mechanism (the fix)
...
### Is the R4 engine even necessary? — the ablation (adversarial)
*(`experiments/derisk/ablation/` — R4 vs a dumb "2D nearest-failed-drug" baseline on the same 20-drug assay-ranking task.)*
...
- **Under strict leave-one-out (simulating a genuinely novel candidate with no analog in the failure DB — the actual use case), naive 2D collapses to 0/20 while R4 holds 12/20.** If a candidate already resembles a known failure you don't need *any* tool; the tool exists for the novel molecule, and that is exactly the regime where only class-aggregation works.
- R4 **uniquely wins the singleton/diverse mechanisms** — taranabant (only 0.15 similar to its lone CB1 partner, yet R4 nails it at z=8.6, rank 1), alosetron (5-HT3), mibefradil (Cav1.2). An aggregation control (compare to *one* random ChEMBL active instead of the class) scores just 2/20 — confirming it's the **diverse-class aggregation** that carries R4, not merely "having ChEMBL data."

**The claim this licenses (state it exactly, don't overreach):** *"For a novel candidate with no analog among known failures, pairwise similarity buries the liability at rank ~12 and only class-aggregation recovers it to the top (naive 0/20 → R4 12/20 under strict hold-out)."* Do **not** claim R4 beats similarity search in general — on a reference set with analogs present, it doesn't. This makes the applicability-domain check (below) load-bearing: the tool should **route** — *known analog → cheap similarity suffices; novel → R4 adds the value.*
...
**Blunt answer:** ProTox **partially overlaps** on coarse cardiac tox and is a legitimate **M2 component** (we already wrap it) — but it is **not a substitute** for the linkage-and-assay-triage layer, and it does not make the project redundant. Its structure-gated 15-target ceiling is exactly why a ligand-similarity engine (needs no structure; reaches 37/39 panel targets) is the right tool for the buried liabilities.
...
| **Panel coverage** | **37/39 targets serviceable**; gap: **BSEP 5 actives; mito = no single target** | ✅ broad off-target reach; ❌ liver is a data desert → M2 |
...
|---|---|---|---|
| **M1 — off-target linkage (R4, the validated core)** | cardiac, CNS, GI, endocrine, immune (37 targets) | similarity to known ChEMBL actives → linked clinical failures | **mechanism-linked** (strongest) |
| **M2 — outcome models** | liver DILI, mitochondrial, reactive-metabolite | QSAR on DILIrank, Tox21-mito, structural alerts; wrap ADMET-AI / ProTox | **model-predicted / alert-based** |
| **Fusion** | all | rank + tag every recommendation by evidence tier & confidence; **abstain** or flag *known-analog* outside domain | — |
...
```
  ...
  SMILES  →  │ 1. Multi-rung similarity / linkage           │
             ...
             │   R3  3D shape + pharmacophore (USRCAT)      │  ← test-gated, weak supplement
             │   R4  off-target / mechanism (ChEMBL class)  │  ← THE differentiator (novel chemotypes)
             └───────────────────┬─────────────────────────┘
                                 ▼
             ┌─────────────────────────────────────────────┐
             ...
             │   → M2 outcome-models fuse in for liver/mito │
             └───────────────────┬─────────────────────────┘
                                 ▼
             ┌─────────────────────────────────────────────┐
...
```
- **`safety_related` + `failure_reason` are mandatory** — never tier a compound T1 without a curated toxicity-causal reason + provenance.
- **Label tiers:** **T1** curated toxicity-causal failure; **T2** organ-severity-ranked marketed (DILIrank/DICTrank Most); **T3** noisier flags (SIDER-derived, broad ClinTox).

### Layer 1 — R4 detail (the safety-panel pipeline)
...
- **Clinical-failure grounding *is* a severity signal** — resembling a **T1 clinical failure** at a target ("this hit killed real programs at clinical exposure") outranks resembling a benign ligand. T1 does double duty.
...
Ranking ≈ (hit confidence) × (target severity) × (T1 grounding) × (margin, if known). Tag every recommendation **no-go / counter-screen / monitor**.
...
- **Attribution:** name the *shared mechanism/off-target*, the linked failed drugs, and where structural, the driving substructure (fingerprint-bit→atom mapping for R1/R2; shared pharmacophore for R4). Defends against "isn't this just fingerprint similarity?"
- **LLM agent** turns retrieved evidence into a report (retrieval + reasoning; no training for v1).
- **Visualization** — candidate at center, linked failed drugs around it, **edges labeled by mechanism**, colored by organ; the reordered assay plan with the **assays-to-culprit** delta shown explicitly (default rank → our rank).
...
- **Necessity ablation (done):** R4 vs naive 2D — R4 wins only in the novel-chemotype regime (strict-LOO naive 0/20 vs R4 12/20); report it, don't hide it.
...
- **Honest bounds:** proxies miss metabolism/reactive-metabolites/dose; no value on hERG; R4 only earns its keep on novel chemotypes; T1 positive class small.
...
- [x] R4 class-aggregation recovers the mechanism (terfenadine→hERG z=+6.4).
...
- [x] **Assay-recovery generalizes (n=20):** buried assays-to-culprit **11.3→3.8**, top-3 **7/10 vs 1/10**, **6 non-obvious wins** across 5-HT2B / CB1 / 5-HT3.
...
- [ ] Reference set: SMILES + ChEMBL off-targets + DILIrank/DICTrank organ join; load `experiments/derisk/drugs.json` (20 curated failures) + fixtures + the documented novelty search log.
- [ ] R4 module across the rich panel (class-membership z-scoring, strict leave-one-out, MAX metabolite aggregation).
- [ ] **Evidence-tier + coverage + known-analog flags as first-class output.**
- [ ] **Severity/actionability table** (§4 Layer 3b) with marginal-value emphasis (buried > hERG).
- [ ] mechanism → assay mapping → **reordered plan with the assays-to-culprit delta** displayed.
- [ ] **★ Assays-to-culprit metric wired into the UI** — the headline number, on held-out failures.

**Phase 2 — M2: breadth for the metabolism/phenotypic gap**
- [ ] Wrap outcome-models: DILI QSAR (DILIrank), Tox21 mito model, reactive-metabolite alerts; fold in ADMET-AI / ProTox endpoints (driver ready in `experiments/derisk/protox/`).
- [ ] Fuse M1 + M2 into one plan, each item tagged by tier + confidence; liver/mito shown as *model-predicted*, not mechanism-linked.
...
**Phase 4 — Stretch**
- [ ] Learned-embedding rung (only with demonstrated scaffold-split lift); off-target docking panel; AACT `why_stopped` mining; 3D O3A/ROCS test.

---

## 6. Demo / pitch (investor-facing)

**Claim:** *"For a candidate that's going to die on a safety liability, we run the assay that kills it **first** — cutting the experiments, time, and cost you burn before the go/no-go decision. We do it by linking the molecule to clinical failures by *shared mechanism*, including liabilities no structure search would find."* The headline is a **number: on real historical failures we cut assays-to-culprit from ~11 to ~4.**
...
4. **Assays-to-culprit gain, shown as the headline:** default plan rank → our rank, per drug, with the aggregate **11.3 → 3.8**.
...
1. **T1 label provenance.** Withdrawn ≠ toxicity-caused. → mandatory `failure_reason` + `safety_related` curation before T1 (§4 schema).
2. **Small T1 positive class** (~90–110 in ClinTox). → lean on T2 (DILIrank/DICTrank ~1.3k) + R4 for scoring; T1 as the clinical-grounding badge.
...
6. **The engine only beats dumb similarity on novel chemotypes.** Measured ablation: naive 2D ties/wins when an analog exists; R4 wins (naive 0/20 → R4 12/20) only under strict hold-out. → state the claim conditionally; ship the **`known-analog` flag** so the tool is honest about when it adds value; measured off-targets for reference drugs, predicted only for the candidate.
...
**Off-target / mechanism (R4):** **Bowes 2012 ~44-target panel** (aligned to SAFETYscan47) as the vocabulary; **ChEMBL** actives per target (pChEMBL ≥ 6, IC50/Ki/Kd) via REST; SEA / ChEMBL target-prediction / PIDGIN and per-target QSAR (hERG has strong public models) for the production upgrade; optional docking.
...
**Only-if-lift-shown:** MolFormer/ChemBERTa/Uni-Mol as an added rung, gated by a scaffold-split holdout.
...
- Secondary-pharmacology safety panel (R4 vocabulary) — Bowes et al., *Nat Rev Drug Discov* 2012;11:909
...
|---|---|---|
| 2D / 3D / R4 feasibility | `experiments/` | 2D fails (0.05–0.19); R4 recovers (z=+6.4) |
| Specificity + cross-target + scaffold + coverage | `experiments/` | AUC 0.894/0.95/0.92/0.88; scaffold 0.913/0.667; 37/39 |
| **Assay-recovery n=20 (decisive)** | `derisk/expanded/` | buried assays-to-culprit **11.3→3.8**; 6 non-obvious wins |
| **R4-vs-2D ablation** | `derisk/ablation/` | R4 necessary only for novel chemotypes (naive 0/20 strict-LOO) |
