...
**"assays until culprit" — the killer assay's rank in a default panel vs. in our plan** (e.g. CB1
...
- **The plan & scientific rationale:** [`toxicity-assay-recommender.md`](toxicity-assay-recommender.md) — read §1 (idea), §4 (architecture: off-target class matching, severity/actionability table, mechanism-to-assay map), §6 (demo), §7 (risks).
- **One-pager:** [`SUMMARY.md`](SUMMARY.md).
- **Validated methods & data you REUSE (do not reinvent):**
  - `experiments/score.py` — the off-target class-matching backbone: per target, mean-top5 ECFP4 (Morgan r=2, 2048-bit) Tanimoto to that target's ChEMBL actives, z-scored vs a 24-drug BACKGROUND set (the background dict is in this file — copy it).
...
### [x] Milestone 1 — Core scoring engine (`core.py`)
- `score_candidate(smiles, metabolite_smiles=None)`:
  ...
  2. Per panel target: mean-top5 ECFP4 Tanimoto to `panel_actives`, z vs `background`. **No live-path leave-one-out.** If metabolite given, MAX-aggregate.
...
### [x] Milestone 2 — Ranking + reordered plan + evidence (`core.py` cont.)
- Apply Layer-3b severity re-weight → priority per target; map targets→assays (Layer-4); rank → reordered plan.
- Compute **assays-until-culprit delta**: rank of each flagged assay in `baseline_order` vs. ours.
...
- Render: the reordered assay plan table (assay, our rank, default rank, delta, action tag), the assays-until-culprit headline, the evidence trail, the LLM narrative, and abstain/known analog banners.
...
- Result caching; more reference drugs (scale curation); the assays-until-culprit cost/time weighting (weight assays by real $/turnaround).
...
## 7. DO-NOT list
- Do not rewrite the off-target class-matching math — adapt `exp_score.py`.
- Do not add leave-one-out to the live candidate path (it was only for retrospective validation).
